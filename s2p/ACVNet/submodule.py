...
def build_gwc_volume_test(refimg_fea, targetimg_fea, mindisp, maxdisp, num_groups):
    B, C, H, W = refimg_fea.shape
...
def build_concat_volume_test(refimg_fea, targetimg_fea, mindisp, maxdisp):
    B, C, H, W = refimg_fea.shape
...
def disparity_regression_test(x, mindisp, maxdisp):
    assert len(x.shape) == 4
