...
class ACVNet(nn.Module):
    def __init__(self, mindisp, maxdisp, attn_weights_only, freeze_attn_weights):
    ...
    def forward(self, left, right):
        ...
        if self.freeze_attn_weights:
            with torch.no_grad():
                ...
                gwc_volume = build_gwc_volume_test(features_left["gwc_feature"], features_right["gwc_feature"], self.mindisp // 4, self.maxdisp // 4, self.num_groups)
        ...
        if not self.attn_weights_only:
            ...
            concat_volume = build_concat_volume_test(concat_feature_left, concat_feature_right, self.mindisp // 4, self.maxdisp // 4)
        ...
        if self.training:
            ...
            if not self.freeze_attn_weights:
                ...
                cost_attention = F.upsample(att_weights, [self.maxdisp-self.mindisp, left.size()[2], left.size()[3]], mode='trilinear')
                ...
                pred_attention = disparity_regression_test(pred_attention, self.mindisp, self.maxdisp)
            ...
            if not self.attn_weights_only:
                ...
                cost0 = F.upsample(cost0, [self.maxdisp-self.mindisp, left.size()[2], left.size()[3]], mode='trilinear')
                cost0 = torch.squeeze(cost0, 1)
                pred0 = F.softmax(cost0, dim=1)
                # pred0 = disparity_regression(pred0, self.maxdisp)
                pred0 = disparity_regression_test(pred0, self.mindisp, self.maxdisp)

                # cost1 = F.upsample(cost1, [self.maxdisp, left.size()[2], left.size()[3]], mode='trilinear')
                cost1 = F.upsample(cost1, [self.maxdisp-self.mindisp, left.size()[2], left.size()[3]], mode='trilinear')
                cost1 = torch.squeeze(cost1, 1)
                pred1 = F.softmax(cost1, dim=1)
                # pred1 = disparity_regression(pred1, self.maxdisp)
                pred1 = disparity_regression_test(pred1, self.mindisp, self.maxdisp)

                # cost2 = F.upsample(cost2, [self.maxdisp, left.size()[2], left.size()[3]], mode='trilinear')
                cost2 = F.upsample(cost2, [self.maxdisp-self.mindisp, left.size()[2], left.size()[3]], mode='trilinear')
                cost2 = torch.squeeze(cost2, 1)
                pred2 = F.softmax(cost2, dim=1)
                # pred2 = disparity_regression(pred2, self.maxdisp)
                pred2 = disparity_regression_test(pred2, self.mindisp, self.maxdisp)
