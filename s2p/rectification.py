...
from block_matching import rectify_secondary_tile_only
...
def change_disparity_range(disp, H2, flag="negative"):
    if flag == "positive":
        H2 = np.dot(common.matrix_translation(-disp[0], 0), H2)
        disp_new = (0, disp[1] - disp[0])
    if flag == "negative":
        H2 = np.dot(common.matrix_translation(-disp[1], 0), H2)
        disp_new = (disp[0] - disp[1], 0)
    return disp_new, H2
...
def disparity_range(rpc1, rpc2, x, y, w, h, H1, H2, matches, A=None):
    ...
    # 改变视差范围
    disp, H2 = change_disparity_range(disp, H2, "negative")
...
def rectify_pair(im1, im2, rpc1, rpc2, x, y, w, h, out1, out2, A=None, sift_matches=None,
    ...
    if sift_matches is not None:
        ...
        if cfg['debug']:
            ...
            np.savetxt(os.path.join(out_dir, 'sift_matches_disp.txt'), sift_matches, fmt='%9.3f')
            visualisation.plot_matches(im1, im2, rpc1, rpc2, sift_matches,
                                       os.path.join(out_dir, 'sift_matches_disp.png'),
                                       x, y, w, h)

    # compute disparity range
    disp, H2 = disparity_range(rpc1, rpc2, x, y, w, h, H1, H2, sift_matches, A)
    disp_m, disp_M = disp
    ...
    if disp_M > 0:
        ...
        l_hmargin = hmargin
    if disp_m < 0:
        r_hmargin = np.fabs(disp_m)
    else:
        r_hmargin = hmargin
    T = common.matrix_translation(r_hmargin, vmargin)
    H1, H2 = np.dot(T, H1), np.dot(T, H2)
    ...
    # w_out = w0 + 2*hmargin
    # h_out = h0 + 2*vmargin
    w_out = w0 + l_hmargin + r_hmargin
    h_out = h0 + l_hmargin + r_hmargin

    # apply homographies and do the crops
    common.image_apply_homography(out1, im1, H1, w_out, h_out)
    common.image_apply_homography(out2, im2, H2, w_out, h_out)
