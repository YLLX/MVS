...
def tmpfile(ext=''):
    ...
    fd, out = tempfile.mkstemp(suffix=ext, prefix='s2p_',
                               dir=os.path.expandvars(os.path.join(cfg['out_dir'], cfg['temporary_dir'])))
...
def linear_stretching_and_quantization_8bit(img, p=1):
    ...
    a = np.nanpercentile(img, p)
    b = np.nanpercentile(img, 100 - p)

    m = np.round(255 * (np.clip(img, a, b) - a) / (b - a))

    return m.astype(np.uint8)


def linear_stretching_and_quantization_8bit_tile(full_img, tile_img, p=1):
    a = np.nanpercentile(full_img, p)
    b = np.nanpercentile(full_img, 100 - p)

    m = np.round(255 * (np.clip(tile_img, a, b) - a) / (b - a))
