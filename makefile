...
all: default msmw msmw2 msmw3 sgbm sgbm_opencv asift
...
PROGRAMS = $(addprefix bin/, $(SRCIIO))  # 调用 addprefix 函数
...
clean: clean_homography clean_asift clean_sift clean_imscript clean_msmw\
       ...
       clean_lsd clean_s2p clean_opencv_sgbm
	$(RM) c/*.o bin/* lib/*
	$(RM) -r s2p_tmp
...
clean_opencv_sgbm:; $(RM) -r bin/build_sgbm
...
.PHONY: default all sift sgbm sgbm_opencv msmw tvl1 imscript clean clean_sift\
	...
	clean_mgm_multi clean_lsd clean_s2p test distclean clean_opencv_sgbm
