...
int main(int c, char** v)
{
    ...
    int numDisparities = (int) 16 * ceil((maxdisp - mindisp) / 16.0);
    int preFilterCap = 63;
    int uniquenessRatio = 10;
    int speckleWindowSize = 50;
    int speckleRange = 1;
    bool fullDP = true; // to run the full-scale two-pass dynamic programming algorithm

    StereoSGBM* sgbm =  StereoSGBM::create(mindisp, numDisparities, SADwin, P1, P2, LRdiff, preFilterCap, uniquenessRatio, speckleWindowSize, speckleRange, fullDP);

    fprintf (stderr, "mind:%d maxd:%d win:%d P1:%d P2:%d LR:%d\n", mindisp,
        mindisp + numDisparities, SADwin, P1, P2, LRdiff);
    fprintf (stderr, "fullDP:%d preFilterCap:%d uniquenessRatio:%d"
        "speckleWin:%d SpeckleRange:%d\n", fullDP, preFilterCap, uniquenessRatio, speckleWindowSize, speckleRange);
